...
public class MyPrinter implements PrintInterface{
    public void printList(ArrayList<Item> list) {
        ...
        for (Item i : list) {
            if (i != null) {
                ...
                String costColumn = blank.repeat(1) + User.df.format(i.getCost()) + blank.repeat(5 - Double.toString(i.getCost()).length());
                String quantityColumn = blank.repeat(1) + i.getQuantity() + blank.repeat(5 - Integer.toString(i.getQuantity()).length());
    ...
    public void printNonPurchasedList(Item[] list) {
        System.out.println("---------------------------------------------------------------");
        System.out.printf("%-13s %-10s %-10s %-7s %-10s%n","| Category", "| Description","| Priority", "| Price", "| Quantity |" );
        System.out.println("---------------------------------------------------------------");
        ...
        for (Item i : list) {
            if (i != null) {
                ...
                String priorColumn = blank.repeat(5) + i.getPriority() + blank.repeat(7 - Double.toString(i.getPriority()).length());
                String costColumn = blank.repeat(1) + User.df.format(i.getCost()) + blank.repeat(5 - Double.toString(i.getCost()).length());
                String quantityColumn = blank.repeat(1) + i.getQuantityNotPurchased() + blank.repeat(5 - Integer.toString(i.getQuantityNotPurchased()).length());
